...
namespace byhj
{
	void D3DBitmap::init(ID3D11Device *pD3D11Device, ID3D11DeviceContext *pD3D11DeviceContext, HWND hWnd, WCHAR *texFile)
	{
	...
	void D3DBitmap::render(ID3D11DeviceContext *pD3D11DeviceContext, const XMFLOAT4X4 &Model,
	...
	void D3DBitmap::setPos(int sw, int sh, int posX, int posY, int width, int height)
	{
		m_posX  = posX;
		m_posY  = posY;
		m_width = width;
	...
	void D3DBitmap::init_shader(ID3D11Device *pD3D11Device, HWND hWnd)
	{
		HRESULT result;
		...
		D3DRTTShader.attach(D3D_VERTEX_SHADER, L"Bitmap.vsh", "BitmapVS", "vs_5_0");
		D3DRTTShader.attach(D3D_PIXEL_SHADER,  L"Bitmap.psh", "BitmapPS", "ps_5_0");
		D3DRTTShader.end();
