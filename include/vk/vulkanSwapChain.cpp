...
namespace byhj {
    ...
    void VulkanSwapChain::init(void* platformWindow, void* platformHandle)
    {
    	...
    	{
    		fpGetPhysicalDeviceSurfaceSupportKHR(m_physicalDevice, i,
    			m_surface,
    			&supportsPresent[i]);
    	}
    	...
    	err = fpGetPhysicalDeviceSurfaceFormatsKHR(m_physicalDevice,  m_surface, &formatCount, surfFormats);
    	assert(!err);
    ...
    void VulkanSwapChain::setup(VkCommandBuffer cmdBuffer, uint32_t *width, uint32_t *height)
    {
    	...
    	err = fpGetPhysicalDeviceSurfacePresentModesKHR(m_physicalDevice,  m_surface, &presentModeCount, presentModes);
    	assert(!err);
    	...
    	err = fpCreateSwapchainKHR(m_device, &swapchainCI, nullptr, &m_swapChain);
    	assert(!err);
    
    	// If we just re-created an existing m_swapChain, we should destroy the old
    	// m_swapChain at this point.
    	// Note: destroying the m_swapChain also cleans up all its associated
    	...
    	err = fpGetSwapchainImagesKHR(m_device, m_swapChain, &m_imageCount, NULL);
    	assert(!err);
    
    	m_pSwapChainImages = (VkImage*)malloc(m_imageCount * sizeof(VkImage));
    	assert(m_pSwapChainImages);
    	err = fpGetSwapchainImagesKHR(m_device, m_swapChain, &m_imageCount, m_pSwapChainImages);
    	assert(!err);
    
    	m_pBuffers = (SwapChainBuffer*)malloc(sizeof(SwapChainBuffer)*m_imageCount);
    	assert(m_pBuffers );
    	...
    	for (uint32_t i = 0; i < m_imageCount; i++)
    	{
    		...
    		m_pBuffers[i].image = m_pSwapChainImages[i];
    		...
    		colorAttachmentView.image =m_pBuffers[i].image;
    
    		err = vkCreateImageView(m_device, &colorAttachmentView, nullptr, &m_pBuffers[i].view);
    		assert(!err);
    	}
    }
    
    // Acquires the next image in the swap chain
    VkResult VulkanSwapChain::acquireNextImage(VkSemaphore presentCompleteSemaphore, uint32_t *currentBuffer)
    {
    	return fpAcquireNextImageKHR(m_device, m_swapChain, UINT64_MAX, presentCompleteSemaphore, (VkFence)nullptr, currentBuffer);
    }
