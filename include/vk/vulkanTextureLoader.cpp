...
void VulkanTextureLoader::loadTexture(const char* filename, VkFormat format, Vulkan::Texture *texture, bool forceLinear)
{
	...
	if (useStaging)
	{
		...
		Vulkan::setImageLayout(cmdBuffer,
			mappableImage,
		...
		VkSubmitInfo submitInfo = Vulkan::Init::submitInfo();
...
void VulkanTextureLoader::destroyTexture(Vulkan::Texture texture)
{
...
void VulkanTextureLoader::loadCubemap(const char* filename, VkFormat format, Vulkan::Texture *texture)
{
	VkFormatProperties formatProperties;
	VkResult err;
	...
	VkImageCreateInfo imageCreateInfo = Vulkan::Init::imageCreateInfo();
	imageCreateInfo.imageType = VK_IMAGE_TYPE_2D;
	imageCreateInfo.format = format;
	imageCreateInfo.extent ={ texture->width, texture->height, 1 };
	imageCreateInfo.mipLevels = 1;
	...
	VkMemoryAllocateInfo memAllocInfo = Vulkan::Init::memoryAllocateInfo();
	VkMemoryRequirements memReqs;
	...
	// Create sampler
	VkSamplerCreateInfo sampler = Vulkan::Init::samplerCreateInfo();
	sampler.magFilter = VK_FILTER_LINEAR;
	sampler.minFilter = VK_FILTER_LINEAR;
	sampler.mipmapMode = VK_SAMPLER_MIPMAP_MODE_LINEAR;
	...
	// Create image view
	VkImageViewCreateInfo view = Vulkan::Init::imageViewCreateInfo();
	view.image = VK_NULL_HANDLE;
