...
namespace byhj
{
	void Plane::init(ID3D11Device *pD3D11Device, ID3D11DeviceContext *pD3D11DeviceContext)
	{
		               ...
		               const XMFLOAT4X4 &view, const XMFLOAT4X4 &proj)
	{
		...
		auto pIceTexture  = TextureMgr::getInstance()->getD3DTextureByName("metal001.dds");
	...
	void Plane::init_texture(ID3D11Device *pD3D11Device)
	{
		...
		assert(hr == S_OK);
		TextureMgr::getInstance()->loadD3DTexture(pD3D11Device, "metal001.dds");
		TextureMgr::getInstance()->loadD3DTexture(pD3D11Device, "grate.dds");
	}


	void Plane::init_shader(ID3D11Device *pD3D11Device)
	{
		HRESULT result;
		...
		PlaneShader.attach(D3D_VERTEX_SHADER, L"light.vsh", "VS", "vs_5_0");
		PlaneShader.attach(D3D_PIXEL_SHADER, L"light.psh", "PS", "ps_5_0");
		PlaneShader.end();

	}

	void Plane::loadModel(std::string filename)
	{
		std::ifstream fin;
		char ch;
		int i;
