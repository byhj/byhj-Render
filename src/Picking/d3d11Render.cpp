...
namespace byhj
{
	...
	void D3D11Render::v_init()
	{
		...
		m_picking.InitPicking(sw, sh, m_cube.getVertexData(), m_cube.getIndexData());
	...
	void D3D11Render::drawInfo()
	{
		...
		if (pick) {
		  m_Font.render(m_pD3D11DeviceContext, L"Pick True", 22.0f, 10.0f, 20.0f);
		} else {
		  m_Font.render(m_pD3D11DeviceContext, L"Pick false", 22.0f, 10.0f, 20.0f);
		}
		drawfps();
