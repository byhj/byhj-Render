...
void VulkanBase::prepare()
{
	...
	textureLoader = new VulkanTextureLoader(physicalDevice, device, queue, cmdPool);
}

VkPipelineShaderStageCreateInfo VulkanBase::loadShader(const char * fileName, VkShaderStageFlagBits stage)
{
	VkPipelineShaderStageCreateInfo shaderStage ={};
	shaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
	shaderStage.stage = stage;
	shaderStage.module = Vulkan::loadShader(fileName, device, stage);
...
VkPipelineShaderStageCreateInfo VulkanBase::loadShaderGLSL(const char * fileName, VkShaderStageFlagBits stage)
{
	VkPipelineShaderStageCreateInfo shaderStage ={};
	shaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
	shaderStage.stage = stage;
	shaderStage.module = Vulkan::loadShaderGLSL(fileName, device, stage);
	shaderStage.pName = "main"; // todo : make param
	assert(shaderStage.module != NULL);
	shaderModules.push_back(shaderStage.module);
	return shaderStage;
	...
	VkDeviceMemory *memory)
{
	...
	VkBufferCreateInfo bufferCreateInfo = Vulkan::Init::bufferCreateInfo(usage, size);
...
void VulkanBase::loadMesh(
	const char * filename,
	Vulkan::Mesh::MeshBuffer * meshBuffer,
	std::vector<Vulkan::Mesh::VertexLayout> vertexLayout,
	float scale)
{
...
{
	VkCommandBufferBeginInfo cmdBufInfo = Vulkan::Init::commandBufferBeginInfo();

	VkResult vkRes = vkBeginCommandBuffer(postPresentCmdBuffer, &cmdBufInfo);
	assert(!vkRes);

	VkImageMemoryBarrier postPresentBarrier = Vulkan::postPresentBarrier(image);
...
VulkanBase::~VulkanBase()
{
	...
	if (enableValidation)
	{
		Vulkan::Debug::freeDebugCallback(instance);
	}
...
void VulkanBase::initVulkan(bool enableValidation)
{
	...
	VkBool32 validDepthFormat = Vulkan::getSupportedDepthFormat(physicalDevice, &depthFormat);
...
void VulkanBase::setupDepthStencil()
{
	...
	assert(!err);
	Vulkan::setImageLayout(setupCmdBuffer, depthStencil.image, VK_IMAGE_ASPECT_DEPTH_BIT, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
