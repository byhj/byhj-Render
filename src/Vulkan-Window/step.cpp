...
VkResult init_global_layer_properties(Sample &info) {
    ...
    for (uint32_t i = 0; i < instance_layer_count; i++) {
        Vulkan::LayerProperties layer_props;
...
VkResult init_device_extension_properties(Sample &info,
                                          Vulkan::LayerProperties &layer_props) {
...
VkResult init_instance(Sample &info, char const *const appName)
{
	//Vulkan app infomation
	...
	//Vulkan Instance Create Information
	VkInstanceCreateInfo instanceInfo = {};
	instanceInfo.sType = VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO;
	instanceInfo.pNext = NULL;
	instanceInfo.flags = 0;
	instanceInfo.pApplicationInfo = &appInfo;
	instanceInfo.enabledLayerCount = info.instance_layer_names.size();
	instanceInfo.ppEnabledLayerNames = info.instance_layer_names.size() ? info.instance_layer_names.data() : NULL;
    instanceInfo.enabledExtensionCount = info.instance_extension_names.size();
	instanceInfo.ppEnabledExtensionNames = info.instance_extension_names.data();

	VkResult res = vkCreateInstance(&instanceInfo, NULL, &info.instance);
...
VkResult init_device(Sample &info)
{
	...
	VkResult res = vkCreateDevice(info.gpus[0], &device_info, NULL, &info.device);
	assert(res == VK_SUCCESS);
...
VkResult init_enumerate_device(Sample &info, uint32_t gpu_count) {
...
VkResult init_debug_report_callback(Sample &info,
    ...
    info.dbgCreateDebugReportCallback =
        (PFN_vkCreateDebugReportCallbackEXT)vkGetInstanceProcAddr(
            info.instance, "vkCreateDebugReportCallbackEXT");
    ...
    info.dbgDestroyDebugReportCallback =
        (PFN_vkDestroyDebugReportCallbackEXT)vkGetInstanceProcAddr(
            info.instance, "vkDestroyDebugReportCallbackEXT");
    ...
    res = info.dbgCreateDebugReportCallback(info.instance, &create_info, NULL,
...
void init_window_size(Sample &info, int32_t default_width,
...
void init_depth_buffer(Sample &info) {
	...
	res = vkCreateImageView(info.device, &view_info, NULL, &info.depth.imageView);
	assert(res == VK_SUCCESS);
}
...
void init_swap_chain(Sample &info) {
    ...
    for (uint32_t i = 0; i < info.swapchainImageCount; i++) {
       Vulkan::SwapChainBuffer sc_buffer;
...
void init_uniform_buffer(Sample &info) {
    ...
    buf_info.flags = 0;
    res = vkCreateBuffer(info.device, &buf_info, NULL, &info.uniformData.buffer);
    assert(res == VK_SUCCESS);

    VkMemoryRequirements mem_reqs;
    vkGetBufferMemoryRequirements(info.device, info.uniformData.buffer,
                                  &mem_reqs);
    ...
    res = vkAllocateMemory(info.device, &alloc_info, NULL,
                           &(info.uniformData.memory));
    assert(res == VK_SUCCESS);

    uint8_t *pData;
    res = vkMapMemory(info.device, info.uniformData.memory, 0, mem_reqs.size, 0,
                      (void **)&pData);
    assert(res == VK_SUCCESS);
    ...
    res = vkBindBufferMemory(info.device, info.uniformData.buffer,
                             info.uniformData.memory, 0);
    assert(res == VK_SUCCESS);
...
void init_descriptor_and_pipeline_layouts(Sample &info,
                                          bool use_texture) {
...
void init_renderpass(Sample &info, bool include_depth, bool clear) {
...
void init_framebuffers(Sample &info, bool include_depth) {
    ...
    attachments[1] = info.depth.imageView;
    ...
    for (i = 0; i < info.swapchainImageCount; i++) {
        attachments[0] = info.buffers[i].imageView;
...
void init_vertex_buffer(Sample &info, const void *vertexData,
    ...
    buf_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    buf_info.flags = 0;
    res = vkCreateBuffer(info.device, &buf_info, NULL, &info.vertexBuffer.buffer);
    assert(res == VK_SUCCESS);
    ...
    res = vkAllocateMemory(info.device, &alloc_info, NULL,
                           &(info.vertexBuffer.memory));
    assert(res == VK_SUCCESS);
	info.vertexBuffer.bufferInfo.range = mem_reqs.size;
	info.vertexBuffer.bufferInfo.offset = 0;

    uint8_t *pData;
    res = vkMapMemory(info.device, info.vertexBuffer.memory, 0, mem_reqs.size, 0,
                      (void **)&pData);
    assert(res == VK_SUCCESS);
    ...
    res = vkBindBufferMemory(info.device, info.vertexBuffer.buffer,
                             info.vertexBuffer.memory, 0);
    assert(res == VK_SUCCESS);
...
void init_shaders(Sample &info, const char *vertShaderText,
...
void init_pipeline(Sample &info, VkBool32 include_depth,
                   VkBool32 include_vi) {
    VkResult U_ASSERT_ONLY res;
...
void init_image(Sample &info, Vulkan::Texture &texObj,
    ...
    if (!read_ppm(filename.c_str(), texObj.texWidth, texObj.texHeight, 0,
                  NULL)) {
    ...
    image_create_info.extent.width = texObj.texWidth;
    image_create_info.extent.height = texObj.texHeight;
    ...
    if (!read_ppm(filename.c_str(), texObj.texWidth, texObj.texHeight,
    ...
    if (!needStaging) {
        ...
        /* allocate memory */
        res = vkAllocateMemory(info.device, &mem_alloc, NULL, &texObj.memory);
        assert(res == VK_SUCCESS);

        /* bind memory */
        res = vkBindImageMemory(info.device, texObj.image, texObj.memory, 0);
        assert(res == VK_SUCCESS);
        ...
        copy_region.extent.width = texObj.texWidth;
        copy_region.extent.height = texObj.texHeight;
    ...
    res = vkCreateImageView(info.device, &view_info, NULL, &texObj.imageView);
    assert(res == VK_SUCCESS);
}

void init_texture(Sample &info, const char *textureName) {
    Vulkan::Texture texObj;
    ...
    info.textureData.imageInfo.imageView = info.textures.back().imageView;
    info.textureData.imageInfo.sampler = info.textures.back().sampler;
    info.textureData.imageInfo.imageLayout = VK_IMAGE_LAYOUT_GENERAL;
}
...
void init_submit_info(Sample &info, VkSubmitInfo &submit_info,
...
void init_clear_color_and_depth(Sample &info,
...
void init_render_pass_begin_info(Sample &info,
...
void execute_queue_cmdbuf(Sample &info,
...
void destroy_uniform_buffer(Sample &info) {
	vkDestroyBuffer(info.device, info.uniformData.buffer, NULL);
	vkFreeMemory(info.device, info.uniformData.memory, NULL);
}
...
void destroy_depth_buffer(Sample &info) {
	vkDestroyImageView(info.device, info.depth.imageView, NULL);
	vkDestroyImage(info.device, info.depth.image, NULL);
	vkFreeMemory(info.device, info.depth.memory, NULL);
}

void destroy_vertex_buffer(Sample &info) {
	vkDestroyBuffer(info.device, info.vertexBuffer.buffer, NULL);
	vkFreeMemory(info.device, info.vertexBuffer.memory, NULL);
}

void destroy_swap_chain(Sample &info) {
	for (uint32_t i = 0; i < info.swapchainImageCount; i++) {
		vkDestroyImageView(info.device, info.buffers[i].imageView, NULL);
	}
...
void destroy_renderpass(Sample &info) {
	vkDestroyRenderPass(info.device, info.renderPass, NULL);
}
...
void destroy_textures(Sample &info) {
	for (size_t i = 0; i < info.textures.size(); i++) {
		...
		vkDestroyImageView(info.device, info.textures[i].imageView, NULL);
