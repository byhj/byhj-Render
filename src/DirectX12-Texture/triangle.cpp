...
namespace byhj {
	...
	void Triangle::init_texture(ComPtr<ID3D12Device> pD3D12Device) {
		...
		pD3D12Device->CreateCommittedResource(
			...
			D3D12_RESOURCE_STATE_GENERIC_READ,
			nullptr,
			IID_PPV_ARGS(m_pTexture.ReleaseAndGetAddressOf()));

		CreateDDSTextureFromFile(pD3D12Device, "d3d12.dds", m_pTexture, );
		...
		pD3D12Device->CreateSampler(&samplerDesc, m_pSamplerDescHeap->GetCPUDescriptorHandleForHeapStart());
	}
}