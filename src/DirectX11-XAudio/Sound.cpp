...
void Sound::Play()
{
	...
	if (FAILED(hr = PlayWave(m_pXAudio2, L"Media\\Wavs\\MusicMono.wav")))
	{
	...
	if (FAILED(hr = PlayWave(m_pXAudio2, L"Media\\Wavs\\MusicMono_adpcm.wav")))
	{
		wprintf(L"Failed creating source voice: %#X\n", hr);
		SAFE_RELEASE(m_pXAudio2);
		CoUninitialize();
	...
	if (FAILED(hr = PlayWave(m_pXAudio2, L"Media\\Wavs\\MusicSurround.wav")))
	{
		wprintf(L"Failed creating source voice: %#X\n", hr);
		SAFE_RELEASE(m_pXAudio2);
		CoUninitialize();
...
HRESULT Sound::PlayWave(IXAudio2* pXaudio2, LPCWSTR szFilename)
{
	//
